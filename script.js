// === УЛУЧШЕННАЯ ВЕРСИЯ С ПОВТОРНЫМИ ПОПЫТКАМИ, ТАЙМАУТАМИ И ОБРАБОТКОЙ 429 ===
// Обратите внимание: нет никаких автоматических вызовов скачивания. Все обработчики навешиваются только после загрузки DOM.
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const fetchWithTimeout = async (url, options = {}, timeout = 10000) => {
        ...
        try {
            ...
            clearTimeout(id);
            if (error.name === 'AbortError') throw new Error(`Превышен таймаут (${timeout}мс) для ${url}`);
            throw error;
        }
    };

    const retryFetch = async (fn, retries = 3, baseDelay = 1000) => {
    ...
    const sessionCache = { keys: null, accountData: null, installId: null, fcmToken: null };

    const generateRandomString = (length) => {
        if (sessionCache.installId && sessionCache.installId.length === length) return sessionCache.installId;
        const randomString = Array.from({ length }, () =>
            'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'.charAt(Math.floor(Math.random() * 62))
        ).join('');
        if (length === 22) sessionCache.installId = randomString;
        return randomString;
    };

    const fetchKeys = async (primaryOnly = false) => {
        if (sessionCache.keys) return sessionCache.keys;
        const endpoints = [
            'https://keygen.warp-generator.workers.dev',
            'https://warp-generation.vercel.app/keys',
            'https://warp-key-generator.vercel.app/api/keys',
            'https://warp-generator.herokuapp.com/keys'
        ];
        let lastError;
        for (let i = 0; i < endpoints.length; i++) {
            try {
                const response = await retryFetch(() => fetchWithTimeout(endpoints[i]));
                if (response.status === 429) {
                    throw new Error('Слишком много запросов (429). Подождите 15 секунд.');
                }
                ...
                if (!keys.publicKey || !keys.privateKey) throw new Error('Неверный формат ключей');
    ...
    const fetchAccount = async (publicKey, installId, fcmToken, primaryOnly = false) => {
        if (sessionCache.accountData && sessionCache.keys && sessionCache.keys.publicKey === publicKey) {
        ...
        const endpoints = [
            ...
            'https://warp-key-generator.vercel.app/api/wg'
        ];
        let lastError;
        for (let i = 0; i < endpoints.length; i++) {
            try {
                ...
                if (!accountData?.config?.interface?.addresses?.v4 || !accountData?.config?.peers?.[0]?.public_key) {
                    throw new Error('Неполные данные аккаунта');
                }
                sessionCache.accountData = accountData;
                sessionCache.fcmToken = fcmToken;
                return accountData;
            } catch (error) {
                console.warn(`Account endpoint ${endpoints[i]} failed:`, error.message);
                lastError = error;
                if (primaryOnly) break;
                if (i === endpoints.length - 1) throw lastError;
            }
    ...
    const showPopup = (message, type = 'success') => {
        ...
        setTimeout(() => { if (popup.parentNode) popup.parentNode.removeChild(popup); }, 4000);
    };
    ...
    const blockButton = (button, seconds = 15) => {
        if (!button) return;
    ...
    const createAWGHandler = (variant) => {
...
Endpoint = ${randomEndpoint}`;
                ...
                const fileName = variant === 1 ? `WARPm1_${randomNumber}.conf` : (variant === 2 ? `WARPm2_${randomNumber}.conf` : `WARPm3_${randomNumber}.conf`);
                downloadConfig(fileName, wireGuardText);
                showPopup('Конфигурация успешно скачана');
            } catch (error) {
                console.error(error);
                if (error.message.includes('429')) {
                    showPopup('Слишком много запросов. Подождите 15 секунд.', 'error');
                    blockButton(button, 15);
                } else {
                    showPopup(`Ошибка: ${error.message}`, 'error');
                }
            } finally {
                button.classList.remove("button--loading");
                if (!button.disabled || !error?.message?.includes('429')) {
                    button.disabled = false;
                }
            }
        };
    ...
    // Подключаем обработчики только после загрузки DOM
    if (AWGm1) AWGm1.addEventListener('click', createAWGHandler(1)('generateButton2'));
    if (AWGm2) AWGm2.addEventListener('click', createAWGHandler(2)('generateButton3'));
    if (AWGm3) AWGm3.addEventListener('click', createAWGHandler(3)('generateButton4'));

    function getSelectedDNS() {
        if (document.getElementById('cf').checked) return "1.1.1.1, 1.0.0.1, 2606:4700:4700::1111, 2606:4700:4700::1001";
        if (document.getElementById('malw').checked) return "84.21.189.133, 193.23.209.189, 2a12:bec4:1460:294::2, 2a01:ecc0:680:120::2";
        if (document.getElementById('xbox').checked) return "176.99.11.77, 80.78.247.254, 31.192.108.180, 2a00:f940:2:4:2::5d1b, 2a00:f940:2:4:2::21ed";
        if (document.getElementById('geohide').checked) return "45.155.204.190, 95.182.120.241, 2a0c:9300:0:54::1";
        if (document.getElementById('comss').checked) return "83.220.169.155, 212.109.195.93, 195.133.25.16, 2a01:230:4:915::2, 2a01:230:4:306::2";
        if (document.getElementById('google').checked) return "8.8.8.8, 8.8.4.4, 2001:4860:4860::8888, 2001:4860:4860::8844";
    ...
    function getSelectedSites() {
        ...
        if (selectedSites.length === 0) return "0.0.0.0/0, ::/0";
        else return selectedSites.map(site => site.ip).join(', ');
    }
    ...
    function openModal() { modal.style.display = "block"; lockBodyScroll(); }
    if (infoBtn) infoBtn.onclick = openModal;
    if (span) span.onclick = function() { modal.style.display = "none"; unlockBodyScroll(); };

    window.onclick = function(event) {
        if (event.target == modal) { modal.style.display = "none"; unlockBodyScroll(); }
    };

    // === Блокировка настроек при выборе блокирующего DNS ===
    function updateToggleState() {
        const toggleCheckbox = document.getElementById('rules');
        const siteCheckboxes = document.querySelectorAll('.Sites .payment-radio');
        let isAnySiteChecked = false;
        siteCheckboxes.forEach(checkbox => { if (checkbox.checked) isAnySiteChecked = true; });
        if (isAnySiteChecked) { toggleCheckbox.disabled = true; toggleCheckbox.checked = false; }
        else { toggleCheckbox.disabled = false; }
    }
    ...
    function getSelectedDNSRadio() {
        ...
        for (let radio of dnsRadios) if (radio.checked) return radio.id;
        return null;
    }

    function isDNSRadio(radio) { return radio.getAttribute('name') === 'payment'; }
    ...
    function resetSelectedSites() {
        ...
        siteCheckboxes.forEach(checkbox => {
            if (checkbox.checked) {
                ...
                if (parentOption) parentOption.classList.remove('payment-option--checked');
            }
        });
    ...
    function updateRadioButtonsState() {
        ...
        const siteCheckboxes = document.querySelectorAll('input[name="sites"]');
        serverRadios.forEach(radio => { radio.disabled = isBlocking; });
        serverOptions.forEach(option => {
            if (isBlocking) { option.style.opacity = '0.5'; option.style.cursor = 'not-allowed'; option.style.pointerEvents = 'none'; }
            else { option.style.opacity = ''; option.style.cursor = ''; option.style.pointerEvents = 'auto'; }
        });
        siteCheckboxes.forEach(checkbox => { checkbox.disabled = isBlocking; });
        ...
        paymentOptions.forEach(option => {
            ...
            if (radio && isDNSRadio(radio)) { option.style.opacity = '1'; option.style.cursor = 'pointer'; option.style.pointerEvents = 'auto'; }
        });
    }
    ...
    function resetServerToDefault() {
        ...
        if (defaultServer) {
            ...
            document.querySelectorAll('.server-option').forEach(option => option.classList.remove('server-option--checked'));
            const parentOption = defaultServer.closest('.server-option');
            if (parentOption) parentOption.classList.add('server-option--checked');
        }
    }
    ...
    const siteCheckboxes = document.querySelectorAll('.Sites .payment-radio');
    siteCheckboxes.forEach(checkbox => checkbox.addEventListener('change', updateToggleState));
    updateToggleState();
});